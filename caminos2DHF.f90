...
! gfortran -Wall -pedantic -std=f95 caminos2DHF.f90 -o caminos2DHF
! ./caminos2DHF
...
PROGRAM caminos2DH
  ...
  INTEGER(4):: paso, pasosmax,frontera
  REAL(4):: x,y,ang,len
  ...
  DO i=1, pasosmax
    ...
    IF (xn>=frontera) x=2*frontera-x
    IF (xn<=-frontera) x=-2*frontera-x
    IF (yn>=frontera) y=2*frontera-y
    IF (yn<=-frontera) y=-2*frontera-y
